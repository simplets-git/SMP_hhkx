...
class SVGTerminalAdapter {
  ...
  static processProjectOutput(result) {
    ...
    if (result.type === 'html_block' && typeof result.message === 'string') {
      console.log('[SVGTerminalAdapter] Processing HTML block for project command');
  ...
  static async loadSVGFromURL(url, options = {}) {
    try {
      ...
      console.error('[SVGTerminalAdapter] Error loading SVG:', error);
      return `<div class="terminal-error">Error loading SVG: ${error.message}</div>`;
    }
  }
}
